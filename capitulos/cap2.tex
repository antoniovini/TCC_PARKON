...
\section{Portais Administrativos Web}
\label{sec:portais-administrativos}

Além dos aplicativos móveis voltados ao usuário final, soluções completas de reserva de vagas exigem interfaces administrativas que permitam aos gestores de estacionamento gerenciar suas operações de forma prática e centralizada. Portais web administrativos complementam o ecossistema mobile ao oferecer funcionalidades como cadastro de estacionamentos, configuração de vagas e preços, acompanhamento de reservas e gestão de colaboradores.

No contexto do Parkon, o portal web foi desenvolvido utilizando React com o framework Next.js, que combina renderização do lado do servidor com a flexibilidade de uma aplicação de página única. Essa escolha tecnológica permite tempos de carregamento reduzidos, melhor indexação por mecanismos de busca e uma experiência de desenvolvimento produtiva, além de facilitar a integração com a mesma \acrshort{api} REST utilizada pelo aplicativo mobile.

A adoção de um portal web separado do aplicativo mobile segue a tendência de plataformas multi-sided, onde diferentes atores do ecossistema (motoristas e gestores) possuem necessidades e fluxos distintos, atendidos por interfaces especializadas que compartilham a mesma infraestrutura de backend.
